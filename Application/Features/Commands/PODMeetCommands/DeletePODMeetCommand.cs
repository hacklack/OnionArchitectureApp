...
{
    public class DeletePODMeetCommand : IRequest<int>
    {
        public int Id { get; set; }
        public class DeletePODMeetCommandHandler : IRequestHandler<DeletePODMeetCommand, int>
        {
            private readonly IApplicationDbContext _context;
            public DeletePODMeetCommandHandler(IApplicationDbContext context)
            {
                _context = context;
            }
            public async Task<int> Handle(DeletePODMeetCommand command, CancellationToken cancellationToken)
            {

                var checkListDetails = _context.checkListDetails.Where(y => y.CheckListTypeChildId == command.Id && y.CheckListTypeId == CheckListType.PODMeetChecklist).ToList();
                if (checkListDetails != null && checkListDetails.Count() > 0)
                {
                    foreach (var item in checkListDetails)
                    {
                        var chklistQuestion = _context.checkListSubjectiveAnswerQuestion.Where(y => y.ChecklistId == item.Id && y.CheckListTypeId == CheckListType.PODMeetChecklist).ToList();
                ...
                var meetMembers = _context.podMeetMembers.Where(m => m.PODMeetId == command.Id).ToList();
                if (meetMembers.Count > 0)
                {
                    foreach (var mm in meetMembers)
                    {
                        _context.podMeetMembers.Remove(mm);
                        await _context.SaveChanges();
                    }
                }

                var meetPermissions = _context.bubbleMeetMemberPermissions.Where(m => m.BubbleMeetId == command.Id && m.MeetTypeId==MeetType.PODMeet).ToList();
                ...
                var meet = await _context.podMeetDetails.Where(m => m.Id == command.Id).FirstOrDefaultAsync();
                if (meet == null)
                    return default;
                _context.podMeetDetails.Remove(meet);
                await _context.SaveChanges();
                return command.Id;
            }
        }
